...
namespace YS_WEB.DAL
{
	...
	public partial class YS_DeliveryDAL
	{
		...
		public bool Delete()
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from YS_Delivery ");
			strSql.Append(" where ");
			SqlParameter[] parameters = {
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public YS_WEB.Model.YS_Delivery DataRowToModel(DataRow row)
		{
			YS_WEB.Model.YS_Delivery model=new YS_WEB.Model.YS_Delivery();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,UserID,UserName,DeliveryName,DeliverPhone,DeliverSheng,DeliverCity,DeliverZipCode,DeliverAddress ");
			strSql.Append(" FROM YS_Delivery ");
			if(strWhere.Trim()!="")
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM YS_Delivery ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from YS_Delivery T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "YS_Delivery";
			parameters[1].Value = "";
